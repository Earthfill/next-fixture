    ...
    return (
      <div>
        ...
        <div className="flex items-center gap-2 mb-2 text-[10px] text-zinc-400">
          <span>News roundup from leading outlets</span>
        </div>
    ...
    <section>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-bold text-zinc-800 flex items-center gap-2">
          ...
          {title}
          <span className="text-xs font-normal text-zinc-400">from leading outlets</span>
        </h2>
        <a href="/news"
        ...
        <a href={featured.url} target="_blank" rel="noopener noreferrer" className="group block">
          <div className="relative overflow-hidden bg-zinc-900">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5 sm:p-8">
              <span className="inline-block text-[10px] font-semibold uppercase tracking-wider text-white/80 bg-[#002b5c] px-2 py-1 mb-2">{featured.source}</span>
              <h3 className="text-xl sm:text-2xl font-bold text-white leading-tight group-hover:underline">{featured.title}</h3>
              {featured.summary && <p className="text-sm sm:text-base text-zinc-300 mt-2 line-clamp-2">{featured.summary}</p>}
