        ...
        <div className="border border-zinc-200 bg-white">
          {displayItems.map((item) => (
            <a key={item.id} href={item.url} target="_blank" rel="noopener noreferrer"
              ...
              {item.imageUrl && <Image src={item.imageUrl} alt="" width={48} height={48} className="h-12 w-12 object-cover rounded shrink-0" />}
      ...
      {featured && (
        <a href={featured.url} target="_blank" rel="noopener noreferrer" className="group block">
          <div className="relative overflow-hidden bg-zinc-900">
            {featured.imageUrl ? (
              <Image src={featured.imageUrl} alt="" fill className="object-cover opacity-80 group-hover:opacity-100 transition-opacity" sizes="(max-width: 768px) 100vw, 50vw" />
            ) : (
        ...
        {displayItems.map((item, i) => {
          ...
          return (
            <a key={item.id} href={item.url} target="_blank" rel="noopener noreferrer"
              ...
              {item.imageUrl && <Image src={item.imageUrl} alt="" width={56} height={56} className="h-14 w-14 sm:h-16 sm:w-16 object-cover rounded shrink-0" />}
              <div className="min-w-0 flex-1">
